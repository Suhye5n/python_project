...
### 어느 소스가 어디서 되는지 (2026-08 실측)

이미지 소스는 **어디서 실행하느냐**에 따라 되고 안 되고가 갈립니다. 클라우드 IP에서 오는 요청을
막거나, 자바스크립트를 실행해야만 내용을 보여주는 사이트들이 있기 때문입니다.

| 소스 | GitHub Actions | 내 컴퓨터 | 막는 방식 |
| --- | --- | --- | --- |
| Behance | ✅ 8장 | ✅ | — |
| 월간디자인 | ✅ 6장 | ✅ | — |
...
기본 설정은 **Actions에서 실제로 되는 것만 켜져 있습니다**(Behance, 월간디자인). 나머지는
`enabled = false` 또는 주석 처리 상태이고, 상황이 되면 그 줄만 바꾸면 됩니다.
...
대표 이미지가 곧 콘텐츠인 상세 페이지에서만 `og_fallback = true`로 켜세요.
